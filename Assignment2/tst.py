...
sorted_stores_list = sorted(top_stores_dict.items(), key=lambda x: x[1], reverse=True)
...
log_txt_file.close()
"""for x in (range(len(log_deals_list))):
    if log_deals_list[x].print_info() is not None:
        print(log_deals_list[x].print_info_file())"""
